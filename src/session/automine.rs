...
pub fn is_minegem(block_id: u16) -> bool {
    ...
    (block_id >= 3995 && block_id <= 4003) || (block_id >= 4101 && block_id <= 4102) || (block_id >= 4154 && block_id <= 4162)
}
    ...
    loop {
        ...
        tokio::select! {
            ...
            _ = tokio::time::sleep(sleep_duration) => {
                ...
                // Portal Sandwich Logic: Find a portal to use as a "sync anchor"
                let portal_pos = {
                    let st = state.read().await;
                    let mut found = None;
                    if let Some(world) = &st.world {
                        for y in 0..world.height as i32 {
                            for x in 0..world.width as i32 {
                                let idx = (y as u32 * world.width + x as u32) as usize;
                                if let Some(&tid) = st.world_foreground_tiles.get(idx) {
                                    if tid == 110 || tid == 1419 { // Entrance or Nether Exit
                                        found = Some((x, y));
                                        break;
                                    }
                                }
                        ...
                        match resolved_path {
                            Some(path) => {
                                if path.len() > 1 {
                                    ...
                                    // EXCEPTION: If the target is a collectable, we can collect from adjacent.
                                    let move_blocked = if is_last_step && is_collectable {
                                        false // Never block path to a collectable — collect from adjacent
                                    } else {
                                    ...
                                    if move_blocked {
                                        ...
                                        if path.len() == 2 {
                                            if is_collectable {
                                                let cid = opt_cid.unwrap();
                                                // Only send if actually within collection radius (2 tiles)
                                                let dx = (target_x - player_x).abs();
                                                let dy = (target_y - player_y).abs();
                                                if dx <= 2 && dy <= 2 {
                                                    let pkts = vec![
                                                    ...
                                                    _logger.warn("automine", Some(&_session_id), format!("STUCK: No path to collectable {cid} at ({target_x},{target_y})"));
                                                }
                                            } else {
                                                // SAFETY GUARD: Never hit the tile we are standing on.
